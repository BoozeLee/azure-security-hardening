...
# Source qwe helper if available for dry-run & debug
if [ -f "${PWD}/scripts/qwe-sh" ]; then
  # shellcheck source=/dev/null
  source "${PWD}/scripts/qwe-sh"
  export QWE_AGENT="azure-sp-create"
fi

# Fallback az_or_dry wrapper
...
if az_or_dry ad app credential reset -h 2>/dev/null | grep -E -- '--keyvault' >/dev/null 2>&1; then
...
if [ "${USE_KEYVAULT_CERT:-false}" = "true" ] && [ -n "$KEYVAULT_NAME" ]; then
  ...
  KV_POLICY=$(az_or_dry keyvault certificate get-default-policy)
  az_or_dry keyvault certificate create --vault-name "$KEYVAULT_NAME" --name "$CERT_NAME" --policy "$KV_POLICY" >/dev/null
  ...
  PFX_BASE64=$(az_or_dry keyvault secret show --vault-name "$KEYVAULT_NAME" --name "$CERT_NAME" --query value -o tsv 2>/dev/null || true)
...
echo "Registering app in Azure AD"
  APP_ID=$(az_or_dry ad app create --display-name "$APP_NAME" --available-to-other-tenants false --query appId -o tsv)
echo "App created: $APP_ID"

echo "Create service principal"
  az_or_dry ad sp create --id "$APP_ID" || true
...
if [ "${USE_KEYVAULT_CERT:-false}" = "true" ] && [ -n "$KEYVAULT_NAME" ]; then
  ...
  if [ -f "${CERT_NAME}.pfx" ]; then
    ...
    az_or_dry ad app credential reset --id "$APP_ID" --cert @${CERT_NAME}.pfx --append >/dev/null
    else
    echo "Attempting to attach KeyVault certificate to app directly"
    if [ "${SUPPORTS_KEYVAULT_BINDING}" = "true" ]; then
      if az_or_dry ad app credential reset --id "$APP_ID" --keyvault "https://$KEYVAULT_NAME.vault.azure.net" --name "$CERT_NAME" --append >/dev/null 2>&1; then
  ...
  az_or_dry ad app credential reset --id "$APP_ID" --cert @${CERT_NAME}.crt --append
fi

if [ -n "$KEYVAULT_NAME" ]; then
  echo "Importing certificate to KeyVault $KEYVAULT_NAME"
  # validate the keyvault exists
  if ! az_or_dry keyvault show --name "$KEYVAULT_NAME" &>/dev/null; then
  ...
  if [ "$CREATED_IN_KEYVAULT" = "true" ]; then
    ...
    az_or_dry keyvault certificate import --vault-name "$KEYVAULT_NAME" --name "$CERT_NAME" --file "${CERT_NAME}.pfx" --password "$PFX_PASSWORD"
  fi
  ...
  if [ "${STORE_PASSWORD_IN_KEYVAULT:-false}" = "true" ]; then
    ...
    az_or_dry keyvault secret set --vault-name "$KEYVAULT_NAME" --name "$SECRET_PFX_NAME" --value "$PFX_PASSWORD" >/dev/null
  fi
  ...
  if [ "${STORE_PASSWORD_IN_KEYVAULT:-false}" = "true" ]; then
    ...
    az_or_dry keyvault secret set --vault-name "$KEYVAULT_NAME" --name "$CERT_NAME_SECRET" --value "$CERT_NAME" >/dev/null
  fi
  ...
  if [ "${STORE_APP_CREDENTIALS_IN_KEYVAULT:-false}" = "true" ]; then
    ...
    az_or_dry keyvault secret set --vault-name "$KEYVAULT_NAME" --name "$APP_CRED_SECRET_NAME" --value "$(cat ./app_credentials.json)" >/dev/null
  fi
  ...
  if [ "${ASSIGN_KEYVAULT_ACCESS_POLICY:-false}" = "true" ]; then
    ...
    SP_OBJECT_ID=$(az_or_dry ad sp show --id "$APP_ID" --query objectId -o tsv)
    if [ -z "$SP_OBJECT_ID" ]; then
      ...
      if az_or_dry keyvault set-policy --name "$KEYVAULT_NAME" --object-id "$SP_OBJECT_ID" --certificate-permissions get import list --secret-permissions get list set --key-permissions get wrapKey unwrapKey >/dev/null 2>&1; then
          ...
          if [ "${ALLOW_KEYVAULT_RBAC_FALLBACK}" = "true" -a "${ASSIGN_KEYVAULT_RBAC}" = "true" ]; then
            ...
            KV_SCOPE=$(az_or_dry keyvault show --name "$KEYVAULT_NAME" --query id -o tsv)
            if az_or_dry role assignment create --assignee-object-id "$SP_OBJECT_ID" --role 'Key Vault Certificates Officer' --scope "$KV_SCOPE" >/dev/null 2>&1; then
...
if [ -f "${CERT_NAME}.crt" ]; then
  ...
  THUMBPRINT=$(az_or_dry ad app credential list --id "$APP_ID" --query "[-1].thumbprint" -o tsv 2>/dev/null || true)
  if [ -z "$THUMBPRINT" ] && [ -n "$KEYVAULT_NAME" ]; then
    # try reading from KeyVault secret if operator stored it
    THUMBPRINT=$(az_or_dry keyvault secret show --vault-name "$KEYVAULT_NAME" --name "spn-${APP_NAME}-thumbprint" --query value -o tsv 2>/dev/null || true)
  fi
fi
...
if [ -n "$KEYVAULT_NAME" ]; then
  ...
  az_or_dry keyvault secret set --vault-name "$KEYVAULT_NAME" --name "$THUMBPRINT_SECRET_NAME" --value "$THUMBPRINT" >/dev/null
fi
if [ -n "$KEYVAULT_NAME" ]; then
